...
// get_absolute_with_fallback attempts to resolve the `supplied_path` path. If `supplied` is null it will
...
static char* get_absolute_with_fallback(char* buffer, const char* supplied_path, const char* fallback_path) {
    const char* path;
...
static int uacc_add_utmp_entry(const char *utmp_path, const char *wtmp_path, const char *username, const char *hostname, const int32_t remote_addr_v6[4], const char *tty_name, const char *id, int32_t tv_sec, int32_t tv_usec) {
    ...
    const char* file = get_absolute_with_fallback(&resolved_utmp_buffer[0], utmp_path, _PATH_UTMP);
    if (utmpname(file) < 0) {
    ...
    const char* wtmp_file = get_absolute_with_fallback(&resolved_wtmp_buffer[0], wtmp_path, _PATH_WTMP);
