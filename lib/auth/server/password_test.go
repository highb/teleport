...
func (s *PasswordSuite) TestChangePasswordWithOTP(c *C) {
	...
	dev, err := auth.NewTOTPDevice("otp", otpSecret, fakeClock.Now())
...
func (s *PasswordSuite) shouldLockAfterFailedAttempts(c *C, req auth.ChangePasswordReq) {
...
func (s *PasswordSuite) prepareForPasswordChange(user string, pass []byte, secondFactorType constants.SecondFactorType) (auth.ChangePasswordReq, error) {
