...
func LocalRegister(id server.IdentityID, authServer *server.Server, additionalPrincipals, dnsNames []string, remoteAddr string) (*server.Identity, error) {
	...
	identity, err := server.ReadIdentityFromKeyPair(keys)
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
type HostCredentials func(context.Context, string, bool, server.RegisterUsingTokenRequest) (*server.PackedKeys, error)
...
func Register(params RegisterParams) (*server.Identity, error) {
	params.setDefaults()
	...
	type registerMethod struct {
		call func(token string, params RegisterParams) (*server.Identity, error)
		desc string
	}
...
func registerThroughProxy(token string, params RegisterParams) (*server.Identity, error) {
	...
	keys, err := params.GetHostCredentials(context.Background(),
		...
		server.RegisterUsingTokenRequest{
			Token:                token,
...
func registerThroughAuth(token string, params RegisterParams) (*server.Identity, error) {
	var client *Client
	var err error
	...
	keys, err := client.RegisterUsingToken(server.RegisterUsingTokenRequest{
		Token:                token,
		HostID:               params.ID.HostUUID,
		NodeName:             params.ID.NodeName,
		Role:                 params.ID.Role,
...
func pinRegisterClient(params RegisterParams) (*Client, error) {
	...
	tlsConfig.InsecureSkipVerify = true
	tlsConfig.Time = params.Clock.Now
	authClient, err := New(client.Config{
		Addrs: utils.NetAddrsToStrings(params.Servers),
		Credentials: []client.Credentials{
			client.LoadTLS(tlsConfig),
		},
...
func ReRegister(params ReRegisterParams) (*server.Identity, error) {
	hostID, err := params.ID.HostID()
	if err != nil {
		return nil, trace.Wrap(err)
	}
	keys, err := params.Client.GenerateServerKeys(server.GenerateServerKeysRequest{
		HostID:               hostID,
		NodeName:             params.ID.NodeName,
